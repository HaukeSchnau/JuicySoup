...
class Npc {
  constructor(map, sprites, initialPos, width, height, maxHealth, speed = 0.002, texts) {
    this.map = map;
  ...
  update() {
    ...
    if (Game.player.pos.subV(this.pos).length() < 5 && !this.playerHasMet) {
      this.texts.forEach(text => {
        Game.cutscenes.push(new Cutscene(this.sprites[0], text));
      });
      Game.setCleared(true);
      this.playerHasMet = true;
    }

    const nextDangerousPos = this.pos.add(this.direction === "left" ? 0 : 1, this.height).floor();
    if (!Game.map.get(nextDangerousPos.x, nextDangerousPos.y)) {
      this.speed *= -1;
    }
